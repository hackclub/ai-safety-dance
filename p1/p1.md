...
([:Learn more about "Spaced Repetition" flashcards](../#SpacedRepetition).<!--Alternatively, use [Anki](https://apps.ankiweb.net/) and get [Part One's cards as an Anki deck](TODO)-->)
...
On top of Bengio's suggestions, there have been many other attempts to merge System 1 & 2 in AI: Hybrid AI, Bio-inspired AI, Neuro-symbolic AI, etc...
...
(Bonus: [: a not-comprehensive Who's Who of AI/Safety organizations](#AIOrganizations))
...
[^anthropic]: [From one of the leading AI + AI Safety labs](https://www.anthropic.com/news/anthropics-responsible-scaling-policy): “[We have] consistently found that working with frontier AI models is an essential ingredient in developing new methods to mitigate the risk of AI.”
...
* There's currently legal debate on whether artists & authors can sue AI for theft/plagiarism. The only 3 things I'll say about this are:
    ...
    3. Things can still be legal but very rude, like farting in an elevator.
...
* Whether or not computers can be conscious, I'm pretty sure human neurons are conscious — that's what you & I are "running on" right now. Well, [scientists are currently growing human neurons on chips and training them to do computational tasks.](https://www.technologyreview.com/2023/12/11/1084926/human-brain-cells-chip-organoid-speech-recognition/) I have no mouth and I must scream.
* If a friend I knew died, but "uploaded" themself into a computer — even if I didn't believe the simulation is conscious — I'd still treat their "upload" as my good ol' friend, because 1) I miss them, and 2) It's what they would've wanted.
...
* As Kurt Vonnegut once wrote: *“We are what we pretend to be, so we must be careful about what we pretend to be.”*

Anyway, that's why I always start my ChatGPT conversations with "Hello!" and end with "Thank you, see you later!"
...
Fine. My suspicion is that biologically-inspired AI is the right path — it's worked so far, with artificial neural networks, and convolution (inspired by mammalian visual cortices).

But, counter-argument: some of the most important discoveries in ANNs, in particular backpropagation (how ANNs learn), are completely biologically implausible.

My counter-counter-intuition: Yes, *that's what's holding Deep Learning back.* So many recent advances in DL are basically *getting around* the problems of backpropagation: ResNets, ReLU, Transformers, etc. The human cortex has only 6 layers of neurons. GPT-4 has *120 layers* of neurons, and requires more training data than a human could ever see in thousands of lifetimes.

So, my current "inside view" mental model of AI (subject to future revision, of course) is that our main bottleneck isn't scale. (Also see the Scaling section for why I'm skeptical on scale.) Our main bottleneck is not understanding how human brains work.

But compared to big mysteries like "how does the brain make consciousness" or "why do we sleep & dream", this would "only" require solving smaller mysteries like "how do neurons learn, even when the reward is very far away" or "how do we even recognize the difference between 'red square, yellow triangle' and 'yellow square, red triangle'? Which, surprisingly, has still been unsolved for decades.

I reckon once we solve *those* mysteries, we can "just" simplify & simulate them in computers — like we did with McColloch & Pitts's neurons — then get "AGI".

So *when* will we solve those "small" mysteries, at the speed of our regular, normal science?
...
(If you skipped the flashcards & would like to review them now, click the Table of Contents icon in the right sidebar, then click the "🤔 Review" links.<!--Alternatively, download the [Anki deck for Part One](TODO).-->)
