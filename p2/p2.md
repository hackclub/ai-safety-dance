...
**Update Nov 2025:** A lot of interesting research has happened around "instrumental convergence" since this Part Two was published a bit over a year ago! In no particular order:
...
(technical aside: there's something that *can* happen called "double descent", where if you keep increasing the # of parameters, it'll get worse and worse (overfit) as expected, _but then start getting better again._ as of aug 2024, this phenomenon is not very well-understood.[^double-descent] **update nov 2025:** we understand it a bit better now![^double-descent-explained])
...
(**update nov 2025:** on the flip side, [a study this year](https://arxiv.org/pdf/2502.08640) finds that GPT-4o values a Nigerian life ~13x more than an American life?...)
