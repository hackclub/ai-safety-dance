...
( What about _intelligent_ adversaries, who can steer & navigate through the holes? Look, it's a metaphor. The "multiple imperfect layers of defense" model works even against _much_ more intelligent/capable adversaries. For example, cybersecurity: a strong password + multi-factor authentication + end-to-end encryption = can defend you against well-funded criminals. )
...
It's important to not take the cheese metaphor too literally. The "multiple imperfect layers of defense" model works even against _much_ more intelligent/capable adversaries. For example, cybersecurity: a strong password + multi-factor authentication + end-to-end encryption = can defend you against well-funded criminals.

Further, we can do better than defending against an _already_-misaligned superintelligence. If we're talking about _growing a trusted intelligence from scratch_, then the Swiss Cheese Model still makes sense at each step, *and*, you can use each iteration of a trusted AI as an extra "layer of cheese" for training better iterations of that AI. (See below section: Scalable Oversight)
